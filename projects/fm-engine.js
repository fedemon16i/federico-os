/* FM engine. Uniform scale into .stage. Cursor + tip stay inside .fm-unit / .ey-unit. */
(function (w) {
  var T = [];
  var UNIT_W = 960;
  var UNIT_H = 600;
  ...
  function unitEl() {
    return document.querySelector('.fm-unit, .ey-unit');
  }
  ...
  function cursors(host, n) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      d.style.left = 24 + i * 20 + 'px';
      d.style.top = 36 + i * 18 + 'px';
      host.appendChild(d);
      nodes.push(d);
    }
    return nodes;
  ...
  function clamp(n, el, ox, oy) {
    ...
    n.style.left = Math.max(8, Math.min(UNIT_W - 28, x)) + 'px';
    n.style.top = Math.max(8, Math.min(UNIT_H - 28, y)) + 'px';
  }
  function move(id, el, ox, oy) {
    clamp(typeof id === 'string' ? document.getElementById(id) : id, el, ox, oy);
  }
  ...
  function ring(el, on) {
    if (!el) return;
    el.classList.toggle('fm-ring', on !== false);
  }
  function skin(name) {
