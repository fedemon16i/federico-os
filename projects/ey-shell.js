...
(function (w) {
  ...
  function form(opts) {
    ...
    return '<div class="app">'+nav()+'<div class="lay-form"><div class="lay-pad" style="overflow:hidden">'
      ...
      +'<div style="font-size:11px;color:var(--ey-muted)">Offering customization</div>'
      +'<h3>Workbench</h3>'
      +'<div style="font-size:13px;font-weight:700;margin:0 0 8px">1 · Configure your instance</div>'
      +field('1','Friendly instance name (required)','<div class="box" id="f1">Enter a name…</div>')
      +field('2','Location (required)','<div class="box" id="f2">West 2 ▾</div>')
      +field('4','Point of contact (required)','<div class="box" id="f4">Search people…</div>')
      ...
      +'<div class="ey-disc">Displayed costs are estimates and exclude platform fees.</div>'
      +'<div class="ey-order"><h4>Your order</h4><div style="font-size:12px;color:var(--ey-muted);margin-bottom:8px">Workbench · 1 instance</div><div class="tot">Total <em>$0 / mo</em></div><div style="display:flex;gap:8px;margin-top:10px"><button class="ey-btn ghost" type="button">Cancel</button><button class="ey-btn" type="button">Continue to billing</button></div></div>'
      +help
      +'</div></div></div>';
  }
